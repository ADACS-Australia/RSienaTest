...
void DyadicSetting::initDyadicSetting(const std::map<int, double>& row,
	...
	if (row.find(ego) == row.end()) {
		...
		LOGS(Priority::ERROR)<< "the setting has not been terminated or is used in different contexts";
		throw "the setting has not been terminated or is used in different contexts";
	}
}
